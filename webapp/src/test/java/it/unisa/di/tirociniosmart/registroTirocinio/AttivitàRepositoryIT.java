...
/**
 * Classe che definisce i casi di test per le operazioni sul database inerenti alle attività
...
public class AttivitàRepositoryIT {
	...
	/**
	 * Salva la lista delle attività su database prima dell'esecuzione di ogni singolo test
	 */
	@Before
	public void salvaAttività() {
	 ...
	 /**
	   * Testa l'interazione con il database per il caricamento della lista di attività 
	   * che si trovano in uno stato specificato.
	   * 
	   * @test {@link AttivitàRepository#findAllByStatus(int)}
	   * 
	   * @result Il test è superato se sono caricate solo le attività il cui 
	   *         stato si trova nello stato specificato
	   */
	  @Test
	  public void findAllByStatus() {
	    List<Attività> attivitàValidate = new ArrayList<Attività>();
	    for (Attività attività: listaAttività) {
	      if (attività.getStatus() == Attività.VALIDATA) {
	        attivitàValidate.add(attività);
	      }
	    }
	    
	    // Controlla che la lista delle attività ottenuta dalla repository sia uguale alla lista delle 
	    // domande definite per il test
	    List<Attività> attivitàValidateSalvate = 
	        attivitàRepository.findAllByStatus(Attività.VALIDATA);
	    
	    assertThat(attivitàValidate, everyItem(isIn(attivitàValidateSalvate)));
	  }	
	  
	/**
	* Testa l'interazione con il database per il singolo caricamento delle attività di tirocinio della 
	* lista tramite identificatore.
	* 
	* @test {@link AttivitàRepository#findById(long)}
	* 
	...
	*/	  
	...
	public void findById() {
		for(Attività attività: listaAttività) {
			Attività attivitàSalvata = attivitàRepository.findById(attività.getId());
			assertThat(attività, is(equalTo(attivitàSalvata)));
		}
	}
	
	/**
	* Testa l'interazione con il database per il caricamento della lista di attività di tirocinio dato
	* l'id di una domanda.
	* 
	* @test {@link AttivitàRepository#findAllByDomandaTirocinioId(long)}
	* 
	* @result Il test è superato se sono caricate solo le attività di tirocinio della lista 
	*         relative a quella specifica domanda
	*/
	@Test
	public void findAllByDomandaTirocinio() {		
	    ...
	    List<Attività> attivitàDomandaTirocinio = new ArrayList<Attività>();
	    for(Attività attività : listaAttività) {
	    	if(attività.getDomandaTirocinio().equals(domanda)) {
	    		attivitàDomandaTirocinio.add(attività);
	    		System.out.println(attività.getDescrizioneAttivita());
	    	}
	    	
	    //Controlla che la lista delle attività ottenuta dalla repository
	    //sia uguale alla lista di attività definite per il test
	   List<Attività> attivitàSalvate = attivitàRepository.findAllByDomandaTirocinioId(domanda.getId());
	   assertThat(attivitàDomandaTirocinio, everyItem(isIn(attivitàSalvate)));
	    }		
	}
	
	/**
	* Testa l'interazione con il database per il caricamento della lista di attività di tirocinio dato
	* l'id del registro di tirocinio.
	* 
	* @test {@link AttivitàRepository#findAllByRegistroTirocinioId(long)}
	* 
	* @result Il test è superato se sono caricate solo le attività di tirocinio della lista 
	*         relative a quello specifico registro.
	*/
	@Test
	public void findAllByRegistroTirocinioId() {		
	    //Crea registro di tirocinio
	    RegistroTirocinio registro = new RegistroTirocinio();
	    registro.setStatus(RegistroTirocinio.IN_ATTESA);
	    
	    List<Attività> attivitàDomandaTirocinio = new ArrayList<Attività>();
	    for(Attività attività : listaAttività) {
	    	if(attività.getRegistroTirocinio().equals(registro)) {
	    		attivitàDomandaTirocinio.add(attività);
	    		System.out.println(attività.getDescrizioneAttivita());
	    	}
	    ...
	    //Controlla che la lista delle attività ottenuta dalla repository
	    //sia uguale alla lista di attività definite per il test
	   List<Attività> attivitàSalvate = attivitàRepository.findAllByRegistroTirocinioId(registro.getId());
	   assertThat(attivitàDomandaTirocinio, everyItem(isIn(attivitàSalvate)));
	    }		
	}
	
	@Test
	public void findAllByStatusAndDomandaTirocinioId() {
		DomandaTirocinio domanda = new DomandaTirocinio();
	    domanda.setStatus(DomandaTirocinio.ACCETTATA);
	    domanda.setCfu(8);
	    domanda.setData(LocalDateTime.now());
	    ...
	    List<Attività> attivitàStatusDomanda = new ArrayList<Attività>();
	    for(Attività attività : listaAttività) {
	    	if(attività.getStatus() == Attività.VALIDATA
	    			&& attività.getDomandaTirocinio().equals(domanda)) {
	    		attivitàStatusDomanda.add(attività);
	    	}
	    	
	   //Controlla che la lista delle attività ottenuta dalla repository sia uguale alla
	   //lista delle attività definite per il test
	   List<Attività> attivitàSalvate = 
			   		  attivitàRepository.findAllByStatusAndDomandaTirocinioId(
			   				  Attività.VALIDATA, domanda.getId());
	   assertThat(attivitàStatusDomanda, everyItem(isIn(attivitàSalvate)));
	    }
	}
}
